...
- [ ] **Journey section** (deferred) — when reintroduced, clicking a suggested item starts that skill immediately. Alternatively, the user can schedule it to run automatically after the current step completes
...
- [ ] Custom skills appear alongside built-in skills — the jockey can suggest them in pills
...
When a skill is identified — whether from a pill, the journey section (when reintroduced), or the jockey's pre-turn intent detection — the same skill instructions are injected. The mechanism of triggering makes no difference to the agent.
...
- **Suggestions** are the expected remaining sequence of steps. Linear — "here's what's probably ahead." Example during implementation: Design audit → Code review → Create PR.
...
The journal is the single record of what's happened on a card. The jockey writes journal entries; the primary agent does not manage the journal.
...
## Journey section (deferred)

The journey section — a visual representation of the journal as a connected track of nodes — is designed but not yet surfaced in the UI. The underlying data model (journal entries, scheduled steps, suggestions) is active and drives pill generation. The journey UI will be reintroduced when the right surface for it is identified.
...
- `card-navigation.md` — the topbar status chip, properties popover, and PR bar are UI elements in the card workspace; pills are generated by the jockey rather than hardcoded by status
