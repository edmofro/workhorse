...
The journal is the single record of what's happened on a card, visible in the journey bar. The jockey writes journal entries; the primary agent does not manage the journal. The existing activity log (status changes, commits, file updates) continues to be recorded for auditing but is not surfaced in the UI.
