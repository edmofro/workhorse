...
- `card-navigation.md` — the properties bar (which contains the journey section, From property, and PR chip) is a UI element in the card workspace; pills are now generated by the jockey rather than hardcoded by status
- `branch-context.md` — PR chip and From property in the properties bar; conflict resolution architecture; squash-merge-then-fix flow
