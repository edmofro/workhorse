...
- `card-navigation.md` — the properties bar (which contains the journey section and From property) and the artifacts sidebar (which contains the PR section) are UI elements in the card workspace; pills are now generated by the jockey rather than hardcoded by status
- `branch-context.md` — From property in the properties bar, PR section in the artifacts sidebar; conflict resolution architecture; squash-merge-then-fix flow
